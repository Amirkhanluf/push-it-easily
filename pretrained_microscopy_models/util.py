...
def visualize(images, file_id, output_dir):
    """PLot images in one row."""
    n = len(images)
    os.makedirs(output_dir + f'/{file_id}/', exist_ok=True)
    ...
    for i, (name, image) in enumerate(images.items()):
        ...
        if name != 'mask':
            plt.imsave(output_dir + f'/{file_id}/{name}.png', image, cmap='viridis')
        else:
            assert 2 not in np.unique(image.astype(np.uint8))
            cv2.imwrite(output_dir + f'/{file_id}/{name}.png', image.astype(np.uint8))
